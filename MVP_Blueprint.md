# MVP Blueprint: On-Demand Private Security Coordination Platform

This document outlines the strategic and operational framework for a Minimum Viable Product (MVP) designed to provide on-demand, speed-first private security coordination in South Africa. All decisions are based on the core principles of accessibility, speed, and inclusivity.

---

### 1. Concept Stress-Test & Mitigations

This section identifies key operational risks and the corresponding mitigation strategies embedded into the MVP design.

| Risk Category          | Identified Weakness / Risk                                   | MVP Mitigation Strategy                                                                                                                              |
| ---------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **Physical Access**    | Gaining access to complexes, flats, and standalone homes without keys is a primary bottleneck. | **Layered, Pre-Configured Access:** Users pre-configure their primary access method during onboarding. The platform auto-notifies a designated contact (guard, neighbour) via SMS/call upon dispatch. |
| **Financial Viability**| The "dispatch first, pay later" model is vulnerable to payment defaults. | **Capped Risk & Service Throttling:** The MVP will accept a default rate of <=5%. After one unpaid incident, a user's account is restricted (they can still trigger an alarm but won't get a response) until the balance is settled. This avoids punitive measures while protecting the system. |
...
The platform's primary competitive advantage is speed. The architecture is designed exclusively to minimize the time from panic signal to on-site presence.
...
2.  **Simultaneous Alert & First-to-Accept (T < 30s):**
    *   The platform sends a simultaneous alert to all top responders.
...
2.  **Dispatch - Auto-Notification:**
    *   The moment a responder accepts a dispatch, the platform triggers an automated SMS or phone call to the pre-registered Access Contact(s).
...
1.  **Payment Tiers:**
    ...
    *   **Ultra-Light Subscription (Post-MVP):** A small monthly fee could cover a certain number of dispatches or provide a discount on the per-incident fee.
...
3.  **Default Management:**
    ...
    *   **Acceptable Default Rate:** The MVP financial model will assume a payment default rate of up to 5%.

---

### 5. Final MVP Assumptions Summary
